...
# pytube محذوف من المشروع — نستخدم yt_dlp بدلاً منه
PYTUBE_OK = False
...
async def _run_ydl(opts: dict, url: str, download: bool = True) -> dict:
...
async def _run_wget(url: str) -> str | None:
    """تحميل ملف صغير (thumbnail) في thread منفصل"""
    loop = asyncio.get_running_loop()
    try:
        return await loop.run_in_executor(None, lambda: wget.download(url))
    except Exception:
        return None
...
async def downloader_handler(c: Client, m: Message):
    ...
    k       = botkey()
    channel = await ar.get(f"{DEV_ID}:BotChannel") or "t.me"
    ...
    if (text.startswith("بحث ") or text.startswith("yt ")) and YTSEARCH_OK and PYTUBE_OK:
        ...
        thumb     = await _run_wget(thumb_url) if thumb_url else None
    ...
    if text.startswith("تيك "):
        if await ar.get(f"{cid}:disableTik:{DEV_ID}") or await ar.get(f":disableYT:{DEV_ID}"):
            return
    ...
    if text.startswith("ساوند "):
        if await ar.get(f"{cid}:disableSound:{DEV_ID}") or await ar.get(f":disableYT:{DEV_ID}"):
            return
        query = text.split(None, 1)[1]
        try:
    ...
    if text.endswith("#AUDIO") or text.endswith("#VOICE"):
        ...
        if found and "soundcloud" in found[0]:
            ...
            if await ar.get(cache_k):
                if is_voice:
                    return await m.reply_voice(await ar.get(cache_k))
                return await m.reply_audio(await ar.get(cache_k))
            try:
...
async def yt_get_info(c: Client, query):
    ...
    channel = await ar.get(f"{DEV_ID}:BotChannel") or "t.me"
    url = f"https://youtu.be/{vid_id}"
    # جلب الـ thumbnail من yt-dlp بدلاً من pytube المكسور
    try:
