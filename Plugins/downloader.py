        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(url, params=params, timeout=aiohttp.ClientTimeout(total=12)) as r:
                    ...
                    if r.status == 200 and "application/json" in content_type:
                        data = await r.json()
                        results = []
                        ...
                        if results:
                            logger.info(f"[_yt_search] instance={instance} results={len(results)}")
                            return results
                    else:
                        logger.warning(f"[_yt_search] {instance} status={r.status} ct={content_type}")
        except Exception as e:
            logger.warning(f"[_yt_search] instance={instance} failed: {e}")
            continue
    return []


async def _yt_download(vid_id: str, audio_only: bool = True) -> str | None:
    """تحميل من يوتيوب عبر Invidious"""
    import aiohttp, os
    for instance in INVIDIOUS_INSTANCES:
        try:
            # جلب معلومات الفيديو
            ...
            formats = info.get("adaptiveFormats", []) + info.get("formatStreams", [])
            if audio_only:
                audio_formats = [f for f in formats if f.get("type", "").startswith("audio/")]
                audio_formats.sort(key=lambda x: x.get("bitrate", 0), reverse=True)
                chosen = audio_formats[0] if audio_formats else None
            else:
                video_formats = [f for f in formats if "video" in f.get("type", "") and f.get("qualityLabel")]
                video_formats.sort(key=lambda x: int(x.get("qualityLabel", "0p").replace("p","")), reverse=True)
            ...
            logger.warning(f"[_yt_download] instance={instance} failed: {e}")
            continue
    return None
