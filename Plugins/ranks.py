...
async def ranks_handler(c: Client, m: Message):
    ...
    if text == "قائمة الرتب":
        ...
        for label, rkey in RANK_KEYS.items():
            members = await ar.smembers(f"{cid}:{rkey}s:{DEV_ID}")
            if members:
    ...
    for label, rkey in RANK_KEYS.items():
        ...
        pipe = r.pipeline()
        pipe.set(key, 1)
        pipe.sadd(f"{cid}:{rkey}s:{DEV_ID}", target_id)
        pipe.execute()
        rank_name = await ar.get(f"{cid}:{RANK_LABEL_KEYS.get(label, '')}:{DEV_ID}") or label
    ...
    for label, rkey in RANK_KEYS.items():
        ...
        pipe = r.pipeline()
        pipe.delete(key)
        pipe.srem(f"{cid}:{rkey}s:{DEV_ID}", target_id)
        pipe.execute()
    ...
    if m_rename:
        ...
        await ar.set(f"{cid}:{lkey}:{DEV_ID}", new_name)
...
async def get_ranks_handler(c: Client, m: Message):
    ...
    if text == "المكتومين":
        ...
        members = await ar.smembers(f"{cid}:listMUTEs:{DEV_ID}") or await ar.smembers(f"{cid}:listMUTE:{DEV_ID}")
        if not members:
...
async def del_ranks_handler(c: Client, m: Message):
    ...
    async def _clear_list(rkey: str, title: str, list_key: str, min_checker):
        if not min_checker(uid, cid):
            return await m.reply(f"{k} هذا الأمر يخص ( {title} ) بس")
        members = await ar.smembers(list_key)
        if not members:
            return await m.reply(f"{k} مافيه {title}")
        count = 0
        for mid in list(members):
            ...
            await ar.srem(list_key, mid)
            await ar.delete(f"{cid}:{rkey}:{mid_int}:{DEV_ID}")
            count += 1
