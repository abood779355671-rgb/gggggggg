...
async def show_whisper(c: Client, cb: CallbackQuery):
    ...
    if target_id == "all":
        # همسة للجميع — نحذفها بعد أول فتح لمنع التراكم
        r.delete(f"w:{wid}")
        return await cb.answer(f"🎊 {msg_part[:200]}", show_alert=True)

    if str(viewer) == sender_id or str(viewer) == target_id:
        # إذا المستقبل هو من فتح → أزل الزر وامسح البيانات من Redis
        if str(viewer) == target_id:
            r.delete(f"w:{wid}")  # حذف فوري بعد القراءة — لا داعي للانتظار 24 ساعة
            try:
