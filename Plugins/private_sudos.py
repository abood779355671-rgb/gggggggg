...
async def start_handler(c: Client, m: Message):
    ...
    # ── deep links الهمسة — تُعالج هنا قبل أي شيء آخر ──────────────────
    _args  = m.text.split(None, 1)
    _param = _args[1].strip() if len(_args) > 1 else ""

    if _param == "whisper_help":
        await m.reply(
            "🔒 **طريقة استخدام الهمسة:**\n\n"
            "• في أي مجموعة اكتب:\n"
            "  `@البوت همستك @username`\n\n"
            "• أو افتح خاصي وأرسل يوزر الشخص مباشرة.",
            parse_mode=ParseMode.MARKDOWN,
        )
        return

    if _param.startswith("w_"):
        from Plugins.whisper import _whisper_start_private
        await _whisper_start_private(c, m, _param[2:])
        return
    # ─────────────────────────────────────────────────────────────────────
