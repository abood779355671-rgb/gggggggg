...
# from helpers.utils import group_enabled, can_speak, resolve_text  # غير مستخدمة
...
def _bot_channel() -> str:
    return r.get(f"{DEV_ID}:BotChannel") or "t.me"
...
""",
    ...
    if not r.sismember(f"{DEV_ID}:UsersList", uid):
        r.sadd(f"{DEV_ID}:UsersList", uid)
        uname     = f"@{m.from_user.username}" if m.from_user.username else "ماعنده يوزر"
        total     = r.scard(f"{DEV_ID}:UsersList")
        notif_txt = (
...
async def sudos_private_handler(c: Client, m: Message):
    ...
    # تجاهل إذا لا يملك صلاحيات المطور
    if not is_dev(uid, 0):
        return
    ...
    if text == "الغاء" and any(r.get(k2) for k2 in wait_keys):
        for k2 in wait_keys:
            r.delete(k2)
        return await m.reply(quote=True, text=f"{k} من عيوني لغيت كل شي")

    # ── استقبال القيم في وضع الانتظار ────────────────────────────────────
    if r.get(f"{uid}:setBotName:{uid}:{DEV_ID}"):
        r.delete(f"{uid}:setBotName:{uid}:{DEV_ID}")
        r.set(f"{DEV_ID}:BotName", text)
        return await m.reply(quote=True, text=f"{k} ابشر عيني المطور غيرت اسمي لـ {text}")

    if r.get(f"{uid}:setBotChannel:{uid}:{DEV_ID}"):
        r.delete(f"{uid}:setBotChannel:{uid}:{DEV_ID}")
        r.set(f"{DEV_ID}:BotChannel", text.lstrip("@"))
        return await m.reply(quote=True, text=f"{k} ابشر عيني غيرت قناة السورس لـ {text}")

    if r.get(f"{uid}:setBotKey:{uid}:{DEV_ID}"):
        r.delete(f"{uid}:setBotKey:{uid}:{DEV_ID}")
        r.set(f"{DEV_ID}:botkey", text)
        return await m.reply(quote=True, text=f"{k} ابشر عيني غيرت رمز السورس لـ {text}")

    if r.get(f"{uid}:setDevGroup:{uid}:{DEV_ID}"):
        r.delete(f"{uid}:setDevGroup:{uid}:{DEV_ID}")
        try:
            group_id = int(text)
        except:
            return await m.reply(quote=True, text=f"{k} الايدي غلط!")
        r.set(f"DevGroup:{DEV_ID}", group_id)
        return await m.reply(quote=True, text=f"{k} ابشر عيني قروب المطور لـ {text}")

    if r.get(f"{uid}:setBotOwner:{uid}:{DEV_ID}"):
        r.delete(f"{uid}:setBotOwner:{uid}:{DEV_ID}")
        try:
            get = await c.get_chat(text.lstrip("@"))
        except:
            return await m.reply(quote=True, text=f"{k} اليوزر غلط!")
        r.set(f"{DEV_ID}:owner", get.id)
    ...
    # ── الاحصائيات ───────────────────────────────────────────────────────
    if text == "الاحصائيات":
        users = r.scard(f"{DEV_ID}:UsersList")
        chats = r.scard(f"enablelist:{DEV_ID}")
        return await m.reply(
            quote=True,
    ...
    if text in ("السيرفر", "معلومات السيرفر"):
        ...
        try:
            version = lsb_release.get_distro_information()["DESCRIPTION"] if lsb_release else platform.version()
        except:
    ...
    if text in ("المستخدمين المحظورين", "المحظورين عام"):
        banned = r.smembers(f"listGBAN:{DEV_ID}")
        if not banned:
    ...
    if text == "المجموعات المحظورة":
        chats = r.smembers(f":BannedChats:{DEV_ID}")
        if not chats:
        ...
        async def _do_pv_broadcast():
            ...
            for u in users:
                try:
                    await m.copy(int(u))
                    count += 1
                    await asyncio.sleep(0.05)   # تأخير بسيط لمنع الـ flood
    ...
    if r.get(f"{uid}:gpBroadcast:{DEV_ID}"):
        r.delete(f"{uid}:gpBroadcast:{DEV_ID}")
        chats = list(r.smembers(f"enablelist:{DEV_ID}"))
        rep   = await m.reply("جار الاذاعة..")
    ...
    if text == "جلب نسخة القروبات":
        data  = {"botUsername": _bot_username(c), "botID": c.me.id if c.me else 0, "Chats": [int(x) for x in r.smembers(f"enablelist:{DEV_ID}")]}
    ...
    if text == "جلب نسخة المستخدمين":
        data  = {"botUsername": _bot_username(c), "botID": c.me.id if c.me else 0, "Users": [int(x) for x in r.smembers(f"{DEV_ID}:UsersList")]}
