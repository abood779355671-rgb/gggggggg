...
from helpers.games_data import (
...
def _get_active(cid: int) -> dict | None:
    gtype  = r.get(_gkey(cid, "type"))
    answer = r.get(_gkey(cid, "answer"))
    if not gtype or not answer:
        return None
    return {"type": gtype, "answer": answer}

def _set_active(cid: int, gtype: str, answer: str):
    r.set(_gkey(cid, "type"),   gtype,  ex=GAME_TTL)
    r.set(_gkey(cid, "answer"), answer, ex=GAME_TTL)

def _clear_active(cid: int):
    r.delete(_gkey(cid, "type"))
    r.delete(_gkey(cid, "answer"))

def _add_score(uid: int, cid: int, points: int = 1):
    r.zincrby(f"game_score:{DEV_ID}:{cid}", points, str(uid))
    r.zincrby(f"game_score_global:{DEV_ID}", points, str(uid))

def _get_top(cid: int, n: int = 10):
    return r.zrevrange(f"game_score:{DEV_ID}:{cid}", 0, n - 1, withscores=True)

def _get_user_score(uid: int, cid: int) -> int:
    s = r.zscore(f"game_score:{DEV_ID}:{cid}", str(uid))
...
async def games_handler(c: Client, m: Message):
    ...
    if text in ("الترتيب", "ترتيب الالعاب", "المتصدرين"):
        ...
        for i, (uid_s, score) in enumerate(top):
            try:
                user = await c.get_users(int(uid_s))
    ...
    if text == "مسح النقاط":
        ...
        r.delete(f"game_score:{DEV_ID}:{cid}")
        return await m.reply(f"{k} تم مسح جميع النقاط ✅")

    # ── إنهاء اللعبة ─────────────────────────────────────────────────────
    if text == "انهاء":
        active = _get_active(cid)
        if not active:
    ...
    if m_start:
        ...
        if r.get(f"game_gbangames:{DEV_ID}:{uid}") or r.get(f"{uid}:gbangames:{DEV_ID}"):
