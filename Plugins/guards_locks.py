...
async def _guard_edit_sync(c: Client, m: Message):
    ...
    lock_edit, lock_edit_m, disable_warn = r.mget(edit_keys)

    # قفل التعديل النصي
    if lock_edit and m.text and not is_admin(uid, m.chat.id):
        await m.delete()
        if not disable_warn and not _cooldown_warn(uid, m.chat.id):
...
async def _guard_sync(c: Client, m: Message):
    ...
    if m.text:
        ...
        if banned_words and not is_admin(uid, cid):
    ...
    if lk[8] and m.from_user:
        ...
        if not count:
            ...
            count = int(count)
            if count >= 10:
                r.set(f"{uid}:mute:{cid}{DEV_ID}", 1)
                r.sadd(f"{cid}:listMUTE:{DEV_ID}", uid)
                r.delete(spam_key)
    ...
    if lk[9] and m.via_bot:
        await m.delete()
        if not lk[24] and not _cooldown_warn(uid, cid):
