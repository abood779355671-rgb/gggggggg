...
async def add_msg_count(c: Client, m: Message):
    ...
    if r.get(f"{uid}:mute:{cid}:{DEV_ID}"):
        return
    # r.incr ذري — يمنع race condition ويقلل RTT من 2 إلى 1
    r.incr(f"{DEV_ID}{cid}:TotalMsgs:{uid}")
    r.set(f"{uid}:bankName", m.from_user.first_name[:25])

    # عداد رسائل القروب الكلي
    r.incr(f"{DEV_ID}:TotalGroupMsgs:{cid}")
...
async def add_edited_msg_count(c: Client, m: Message):
    if not m.from_user:
        return
    uid, cid = str(m.from_user.id), str(m.chat.id)
    r.incr(f"{cid}:TotalEDMsgs:{uid}:{DEV_ID}")
...
async def id_handler(c: Client, m: Message):
    ...
    # ── جهات الاتصال ──────────────────────────────────────────────────────
    if text == "جهاتي":
        contacts = int(r.get(f"{cid}TotalContacts{uid}:{DEV_ID}") or 0)
        return await m.reply(f"{k} عدد جهاتك ↢ {contacts}")

    # ── مجموعاتي ──────────────────────────────────────────────────────────
    if text == "مجموعاتي":
        groups = r.smembers(f"{uid}:groups")
        if not groups:
    ...
    if text in ("انشائي", "الانشاء") and not m.reply_to_message:
        return await m.reply(f"{k} الانشاء ( {get_creation_date(uid)} )")

    if text in ("الانشاء", "انشائه") and m.reply_to_message and m.reply_to_message.from_user:
        return await m.reply(f"{k} الانشاء ( {get_creation_date(m.reply_to_message.from_user.id)} )")
    ...
    # ── معلوماتي ──────────────────────────────────────────────────────────
    if text == "معلوماتي":
        msgs  = int(r.get(f"{DEV_ID}{cid}:TotalMsgs:{uid}") or 0)
        edits = int(r.get(f"{cid}:TotalEDMsgs:{uid}:{DEV_ID}") or 0)
...
"""
    ...
    # ── بايو ──────────────────────────────────────────────────────────────
    if text == "بايو":
        if r.get(f"{cid}:disableBio:{DEV_ID}"):
            return
    ...
    # ── افتاري ────────────────────────────────────────────────────────────
    if text == "افتاري":
        if r.get(f"{cid}:disableAV:{DEV_ID}"):
            return
    ...
    if text in ("ايدي", "id") and not m.reply_to_message:
        if r.get(f"{cid}:disableID:{DEV_ID}"):
            return
    ...
    if text in ("رسايله", "رسائلة") and m.reply_to_message and m.reply_to_message.from_user:
        msgs = int(r.get(f"{DEV_ID}{cid}:TotalMsgs:{m.reply_to_message.from_user.id}") or 0)
    ...
    if text in ("مسح رسائلي", "مسح رسايلي"):
        msgs = int(r.get(f"{DEV_ID}{cid}:TotalMsgs:{uid}") or 0)
        r.delete(f"{DEV_ID}{cid}:TotalMsgs:{uid}")
        return await m.reply(f"{k} ابشر مسحت ( {msgs:,} ) من رسائلك")

    # ── مسح تكليجاتي ──────────────────────────────────────────────────────
    if text == "مسح تكليجاتي":
        edits = int(r.get(f"{cid}:TotalEDMsgs:{uid}:{DEV_ID}") or 0)
        r.delete(f"{cid}:TotalEDMsgs:{uid}:{DEV_ID}")
    ...
    if text in ("مسح المتفاعلين", "تصفير المتفاعلين"):
        ...
        keys = list(r.scan_iter(f"{DEV_ID}{cid}:TotalMsgs:*"))
        if keys:
            pipe = r.pipeline()
            for key in keys:
                pipe.delete(key)
    ...
    if text in ("ترتيبي", "تفاعلي"):
        users_keys = list(r.scan_iter(f"{DEV_ID}{cid}:TotalMsgs:*"))
    ...
    # إلغاء وضع الانتظار
    if r.get(f"{cid}:addCustomID:{uid}:{DEV_ID}") and text == "الغاء":
        r.delete(f"{cid}:addCustomID:{uid}:{DEV_ID}")
        return await m.reply(f"{k} ابشر تم الغاء تعيين الايدي")

    if r.get(f"{cid}:addCustomIDG:{uid}:{DEV_ID}") and text == "الغاء":
        r.delete(f"{cid}:addCustomIDG:{uid}:{DEV_ID}")
        return await m.reply(f"{k} ابشر تم الغاء تعيين الايدي عام")

    # استقبال الايدي المخصص (للقروب)
    if r.get(f"{cid}:addCustomID:{uid}:{DEV_ID}") and is_mod(uid, cid):
        r.set(f"{cid}:customID:{DEV_ID}", m.text)
        r.delete(f"{cid}:addCustomID:{uid}:{DEV_ID}")
        return await m.reply(f"{k} وسوينا الايدي\n{k} يمديك تجرب شكل الايدي الجديد الحين")

    # استقبال الايدي المخصص العام
    if r.get(f"{cid}:addCustomIDG:{uid}:{DEV_ID}") and is_dev(uid, cid):
        r.set(f"customID:{DEV_ID}", m.text)
        r.delete(f"{cid}:addCustomIDG:{uid}:{DEV_ID}")
    ...
    if text == "مسح الايدي":
        ...
        if not r.get(f"{cid}:customID:{DEV_ID}"):
            return await m.reply(f"{k} الايدي مو معدل")
        r.delete(f"{cid}:customID:{DEV_ID}")
    ...
    if text in ("مسح الايدي العام", "مسح الايدي عام"):
        ...
        if not r.get(f"customID:{DEV_ID}"):
            return await m.reply(f"{k} الايدي العام مو معدل")
        r.delete(f"customID:{DEV_ID}")
    ...
    if text == "تغيير الايدي":
        ...
        r.set(f"{cid}:customID:{DEV_ID}", template)
        return await m.reply(f"{k} وسوينا الايدي\n{k} يمديك تجرب شكل الايدي الجديد الحين")
    ...
    if text == "تعيين الايدي":
        ...
        await m.reply(guide)
        r.set(f"{cid}:addCustomID:{uid}:{DEV_ID}", 1, ex=300)
        return
    ...
    if text == "تعيين الايدي عام":
        ...
        guide = _id_guide_text()
        await m.reply(guide)
        r.set(f"{cid}:addCustomIDG:{uid}:{DEV_ID}", 1, ex=300)
        return
    ...
    if text == "تعطيل الايدي":
        ...
        r.set(f"{cid}:disableID:{DEV_ID}", 1)
        return await m.reply(f"{k} بواسطة ↤ {m.from_user.mention}\n{k} ابشر عطلت الايدي")
    ...
    if text == "تفعيل افتاري":
        if not is_admin(uid, cid):
            return await m.reply(f"{k} عذراً الامر يخص ↤〖 الادمن 〗فقط .")
        if not r.get(f"{cid}:disableAV:{DEV_ID}"):
            return await m.reply(f"{k} بواسطة ↤ {m.from_user.mention}\n{k} افتار مفعل من قبل")
        r.delete(f"{cid}:disableAV:{DEV_ID}")
        return await m.reply(f"{k} بواسطة ↤ {m.from_user.mention}\n{k} ابشر فعلت افتار")
    ...
    if text == "تعطيل افتاري":
        ...
        if r.get(f"{cid}:disableAV:{DEV_ID}"):
            return await m.reply(f"{k} بواسطة ↤ {m.from_user.mention}\n{k} افتار معطل من قبل")
        r.set(f"{cid}:disableAV:{DEV_ID}", 1)
        return await m.reply(f"{k} بواسطة ↤ {m.from_user.mention}\n{k} ابشر عطلت افتار")
    ...
    if text == "تفعيل الايدي بالصوره":
        if not is_admin(uid, cid):
            return await m.reply(f"{k} عذراً الامر يخص ↤〖 الادمن 〗فقط .")
        if not r.get(f"{cid}:disableIDPHOTO:{DEV_ID}"):
            return await m.reply(f"{k} بواسطة ↤ {m.from_user.mention}\n{k} الايدي بالصوره مفعل من قبل")
        r.delete(f"{cid}:disableIDPHOTO:{DEV_ID}")
        return await m.reply(f"{k} بواسطة ↤ {m.from_user.mention}\n{k} ابشر فعلت الايدي بالصوره")

    if text == "تعطيل الايدي بالصوره":
        if not is_admin(uid, cid):
            return await m.reply(f"{k} عذراً الامر يخص ↤〖 الادمن 〗فقط .")
        if r.get(f"{cid}:disableIDPHOTO:{DEV_ID}"):
            return await m.reply(f"{k} بواسطة ↤ {m.from_user.mention}\n{k} الايدي بالصوره معطل من قبل")
        r.set(f"{cid}:disableIDPHOTO:{DEV_ID}", 1)
...
async def add_contact_count(c: Client, m: Message):
    ...
    for member in m.new_chat_members:
        if m.from_user.id != member.id:  # من أضاف شخصاً آخر
            r.incr(f"{m.chat.id}TotalContacts{m.from_user.id}:{DEV_ID}")
...
async def _send_kashf(c: Client, m: Message, target_id, method: str):
    ...
    try:
        ...
        msgs     = int(r.get(f"{DEV_ID}{cid}:TotalMsgs:{user.id}") or 0)
...
async def _send_id_card(c: Client, m: Message):
    ...
    # جلب القالب
    template = (
        r.get(f"{cid}:customID:{DEV_ID}") or
        r.get(f"customID:{DEV_ID}") or
        DEFAULT_ID_TEMPLATE
    )
    ...
    if r.get(f"{cid}:disableIDPHOTO:{DEV_ID}") or not m.from_user.photo:
    ...
    try:
        ...
        if video:
            ...
            if r.get(cache_key):
                return await m.reply_animation(r.get(cache_key), caption=text)
            file_id_obj = FileId(
            ...
            r.set(cache_key, sent.animation.file_id, ex=3600)
            return
