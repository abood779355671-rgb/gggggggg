...
async def replace_handler(c: Client, m: Message):
    ...
    # إلغاء العملية
    if text == "الغاء" and (r.get(step1) or r.get(step2) or r.get(step3)):
        r.delete(step1, step2, step3)
    ...
    if text in ("استبدال كلمه", "استبدال كلمة"):
        ...
        r.set(step1, 1, ex=600)
        return await m.reply(f"{k} ارسل الكلمة القديمة الآن")

    # الخطوة 1: استلام الكلمة القديمة
    if r.get(step1) and is_botowner(uid, cid):
        r.set(step2, m.text, ex=600)
        r.delete(step1)
        return await m.reply(f"{k} ارسل الكلمة الجديدة الحين")

    # الخطوة 2: استلام الكلمة الجديدة وعرض الملفات
    if r.get(step2) and is_botowner(uid, cid):
        old_word = r.get(step2)
        r.set(step3, f"{old_word}&&new&&{m.text}", ex=600)
        r.delete(step2)
    ...
    if r.get(step3) and is_botowner(uid, cid) and m.text in os.listdir("Plugins"):
