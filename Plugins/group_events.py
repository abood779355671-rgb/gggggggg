...
# كاش معرّف البوت — يُعبأ مرة واحدة عند أول استخدام
...
async def _dev_notify(c, text: str, reply_markup=None):
    ...
    if dev_group:
        ...
        for dev in get_devs():
            try:
                await c.send_message(int(dev), text,
                               disable_web_page_preview=True,
                               reply_markup=reply_markup)
...
async def on_left_member(c: Client, m: Message):
    """عند مغادرة عضو — يعالج كلاً من: مغادرة أعضاء عاديين وطرد البوت"""
    cid = m.chat.id
    k = botkey()

    # ── تحقق إذا كان البوت نفسه من غادر ──
        ...
        # ── البوت أصبح أدمن ──────────────────────────────────────────────
            ...
            if not full_priv:
                ...
                if not r.get(f"{cid}:enable:{DEV_ID}"):
                    ...
                    bot_username = (await c.get_me()).username or ""  # username قد يتغير فلا نخزّنه
